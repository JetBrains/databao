...
from portus.caches.disk_cache import DiskCacheConfig, SqliteDiskCache


@pytest.fixture
def cache(tmp_path: Path) -> SqliteDiskCache:
    config = DiskCacheConfig(db_dir=tmp_path)
    return SqliteDiskCache(config)


def test_set_and_get(cache: SqliteDiskCache) -> None:
...
def test_set_and_get_empty(cache: SqliteDiskCache) -> None:
    sql_text = "SELECT * FROM dummy_table"
    source = "dummy_source"
...
def test_set_and_get_empty_duplicates(cache: SqliteDiskCache) -> None:
    sql_text = "SELECT * FROM dummy_table"
    source = "dummy_source"
...
def test_get_with_no_match(cache: SqliteDiskCache) -> None:
...
def test_invalidate_source_if_stale(cache: SqliteDiskCache) -> None:
...
def test_invalidate_tag_no_match(cache: SqliteDiskCache) -> None:
    source = "nonexistent_source"
