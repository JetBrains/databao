...
class SqlAlchemyDataSourceConfig(DataSourceConfig):
    db_type: str = Field(
        description="Type of the database management system which is going to be queried by the agent or at profiling time. "
        "If a type is used, which is not among the explicitly checked types in the create_db_engine function, a "
        "warning will be raised as the queries generated by sqlite will not have been tested on this database "
    ...
    In that case, no timeout exception will be raised. E.g. `select * from customers` takes >10s with no exception even with a 1s timeout.
    ...
    # TODO: separate from here into a child class DataloreDataSourceConfig as then, we could easily subclass the SfBqDataSourceConfig then, get_url must become abstract
    ###########################
