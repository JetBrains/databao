...
class SqlAlchemyDataSourceConfig(DataSourceConfig):
    db_type: str = Field(
        ...
        "the agent or at profiling time. If a type is used, which is not among the explicitly checked types "
        "in the create_db_engine function, a warning will be raised as the queries generated by sqlite "
        "will not have been tested on this database management system and may produce unexpected failures.",
    ...
    In that case, no timeout exception will be raised. E.g. `select * from customers` takes >10s 
    with no exception even with a 1s timeout. You can check that the timeout actually works with `select sleep(3)`
    """
    ...
    #  we could easily subclass the SfBqDataSourceConfig then, get_url must become abstract
    ###########################
    ...
    """URL to connect to the database. If None, the driver, user, password, host, port, schema, and db_options 
    will be used to construct the URL."""
